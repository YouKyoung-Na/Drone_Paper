...
from v7_utils.datasets import LoadStreams, LoadImages
from v7_utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    scale_coords, xyxy2xywh, strip_optimizer, set_logging, increment_path, save_one_box
from v7_utils.plots import colors, plot_one_box
from v7_utils.torch_utils import select_device, load_classifier, time_synchronized
...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Process detections
        for i, det in enumerate(pred):  # detections per image # 생성된 bbox 수 만큼 반복
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    ...
                    # print(f'\n\n blur_img = {blur_img} \n\n')
                    # print(f'\n\n blur_img.shape = {blur_img.shape} \n\n')


                    if opt.blur:
                        mosaic_loc = blur_img[int(xywh[1])-int((xywh[3]/2)):int(xywh[1])+(int(xywh[3]/2)), int(xywh[0])-(int(xywh[2]/2)):int(xywh[0])+(int(xywh[2]/2))]
                        mosaic_loc = cv2.blur(mosaic_loc, (50,50))
                        
                        original_img = blur_img 
                        original_img[int(xywh[1])-int((xywh[3]/2)):int(xywh[1])+(int(xywh[3]/2)), int(xywh[0])-(int(xywh[2]/2)):int(xywh[0])+(int(xywh[2]/2))] = mosaic_loc
