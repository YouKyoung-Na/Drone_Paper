...
from utilss.datasets import letterbox, img_formats, vid_formats, Face_LoadImages, Face_LoadStreams
from utilss.general import check_img_size, non_max_suppression_face, apply_classifier, scale_coords, xyxy2xywh, \
...
def detect(model, source, device, project, name, exist_ok, save_img, view_img):
    ...
    if webcam:
        ...
        dataset = Face_LoadStreams(source, img_size=imgsz)
        bs = 1  # batch_size
    else:
        print('loading images', source)
        dataset = Face_LoadImages(source, img_size=imgsz)
        bs = 1  # batch_size
